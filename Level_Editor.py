...
bg_img = pygame.image.load('img/bgimg.jpg')
		...
		for col in range(20):
			if world_data[row][col] > 0:
				...
				if world_data[row][col] == 9:
					#bordervert
					img = pygame.transform.scale(bordervert_img, (tile_size // 2 , tile_size * 1.5  ))
					screen.blit(img, (col * tile_size + (tile_size - 7), row * tile_size - (tile_size // 2)))
...
while run:
	...
	if save_button.draw():
		#save level data
		pickle_out = open(f'Data_Files/level{level}_data', 'wb')
